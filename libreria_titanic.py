...
model3 = RandomForestClassifier(n_estimators=20, random_state=3)
model3.fit(x, y)
...
model4 = RandomForestClassifier(n_estimators=20, random_state=4)
model4.fit(x, y)
...
model5 = RandomForestClassifier(n_estimators=20, random_state=5)
model5.fit(x, y)
...
# Hacemos mediciones de accuracy
...
# Imprimimos
print("Accuracy 1:", ac1)
print("Accuracy 2:", ac2)
print("Accuracy 3:", ac3)
print("Accuracy 4:", ac4)
print("Accuracy 5:", ac5)

# Sacamos la media de las mediciones de accuracy
print("The mean of the Accuracy score is", np.mean([ac1, ac2, ac3, ac4, ac5]))

"""Para hacer mediciones, agarramos el modelo con mayor Accuracy Score, el cual fue el modelo 1"""
...
importance = pd.Series(model1.feature_importances_, index=feature_names)
...
# CM
y_pred = cross_val_predict(model1, x, y, cv=10)
sns.heatmap(confusion_matrix(y, y_pred), annot=True, fmt='3.0f')
plt.show()

# Learning curve
train_sizes, train_scores, test_scores = learning_curve(model1, x, y, n_jobs=-1, train_sizes=np.linspace(.1, 1.0, 5), verbose=0)
